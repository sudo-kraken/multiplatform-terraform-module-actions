## This is the main Terraform configuration file for the module vsphere-ws22-module
## Author Joe Harrison
...
data "template_file" "init" {
  ...
  vars = {
    adpass   = var.ws22.ADPass
    adou     = var.ws22.ADOU
    aduser   = var.ws22.ADUser
    addomain = var.ws22.ADDomain
    adjoin   = var.ws22.ADJoin
    gateway  = var.ws22.vm_gateway
  }
}
...
resource "vsphere_virtual_machine" "vm" {
  ...
  num_cpus              = var.ws22.num_cpus
  memory                = var.ws22.memory
  ...
  clone {
    ...
    customize {
      windows_options {
        computer_name    = var.ws22.hostname
        admin_password   = var.ws22.vm_password == "" ? random_password.password.result : var.ws22.vm_password
        workgroup        = "WORKGROUP"
        auto_logon       = true
        auto_logon_count = 2
        time_zone        = var.ws22.vm_timezone
      ...
      network_interface {
        ipv4_address    = var.ws22.vm_ip
        ipv4_netmask    = var.ws22.vm_netmask
        dns_server_list = var.ws22.vm_dns_servers
        dns_domain      = var.ws22.vm_dns_domain
      }

      ipv4_gateway = var.ws22.vm_gateway
