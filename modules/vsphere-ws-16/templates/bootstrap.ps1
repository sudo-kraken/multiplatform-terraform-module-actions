...
# Set Gateway of Ethernet0
$gateway = "${gateway}"
# Get the network adapter
$Adapter = Get-NetAdapter -Name "Ethernet0"
# If the adapter is found, update the gateway
If ($Adapter) {
    # Get existing default gateway
    $ExistingGateway = Get-NetRoute -InterfaceIndex $Adapter.ifIndex -DestinationPrefix "0.0.0.0/0"
    ...
    Write-Output "Gateway updated successfully for Ethernet0."
} Else {
    Write-Output "No adapter found with the name 'Ethernet0'."
}
...
$connections |foreach {
    # Check if the network connection is part of a domain
    $domainRole = (Get-WmiObject Win32_ComputerSystem).DomainRole
    if(1,3,4,5 -contains $domainRole) { 
        Write-Host "Skipping domain-joined network: " $_.GetNetwork().GetName()
        continue 
    }
...
$SkipNetworkProfileCheck = $false  # Or $true, depending on your needs
$CreateSelfSignedCert = $true
$ForceNewSSLCert = $true
$GlobalHttpFirewallAccess = $false  # Or $true
...
# An array of letters for new drives
$DriveLetters = 68..89 | ForEach-Object { [char]$_ }
# Check if there are uninitialised disks
If ($Disks) {
    # Iterate through each disk
    ForEach ($Disk in $Disks) {
        # Initialise the disk (GPT partition style)
        ...
        # Set the drive label
        If ($DriveLetters[0] -eq "D"){
        Set-Volume -DriveLetter $DriveLetters[0] -NewFileSystemLabel "DATA" 
        }
        Else {
        Set-Volume -DriveLetter $DriveLetters[0] -NewFileSystemLabel "$($DriveLetters[0])"
        }
        # Remove the used letter from the array
        $DriveLetters = $DriveLetters | Where-Object { $_ -ne $DriveLetters[0] }
    }
    Write-Output "Disk initialisation process has been completed!"
} Else {
    Write-Output "No uninitialised disks found."
}

# Clear userdata
Write-Host "Clearing userdata..."
