## This is the main Terraform configuration file for the module vsphere-ws16-module
## Author Joe Harrison
...
data "template_file" "init" {
  ...
  vars = {
    adpass   = var.ws16.ADPass
    adou     = var.ws16.ADOU
    aduser   = var.ws16.ADUser
    addomain = var.ws16.ADDomain
    adjoin   = var.ws16.ADJoin
    gateway  = var.ws16.vm_gateway
  }
}
...
resource "vsphere_virtual_machine" "vm" {
  ...
  num_cpus              = var.ws16.num_cpus
  memory                = var.ws16.memory
  ...
  clone {
    ...
    customize {
      windows_options {
        computer_name    = var.ws16.hostname
        admin_password   = var.ws16.vm_password == "" ? random_password.password.result : var.ws16.vm_password
        workgroup        = "WORKGROUP"
        auto_logon       = true
        auto_logon_count = 2
        time_zone        = var.ws16.vm_timezone
      ...
      network_interface {
        ipv4_address    = var.ws16.vm_ip
        ipv4_netmask    = var.ws16.vm_netmask
        dns_server_list = var.ws16.vm_dns_servers
        dns_domain      = var.ws16.vm_dns_domain
      }

      ipv4_gateway = var.ws16.vm_gateway
